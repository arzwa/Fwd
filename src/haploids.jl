# Should work for arbitrary ploidy levels provided low-level functions
# (mainly `_generate_offspring`) are implemented
...
function generation!(
    ...
    @assert "Biparental reproduction" length(idx) == 2N  
    # new nodes to ts
...
Recombine haplotype at `p1` and `p2`, write to `k`, and update the tree sequence.
"""
...
# Functions specialized to particular ploidy levels
