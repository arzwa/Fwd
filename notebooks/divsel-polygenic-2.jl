...
res = deserialize("data/2026-01-06.jls")
Ts = map(res) do X
    Fwd.estimate_coaltimes(getindex.(X, 2), idx=4)
end
...
end |> x->plot(x..., layout=(2,1), size=(700,450))
...
pop, ts, qs = let NA=1000, C=0.25, ngen=200_000
    G   = C*100*10^6   # (1cM/Mb)
    xs  = ceil.(Int64, ys .* G)
...
BP = Equilibrium(BPModel(m, ss, xs .* C, NB, u))

xx, yb, _, _ = tab([ts])
plot(xx, yb, color=:lightgray, yscale=:log10)
nn = 500 
